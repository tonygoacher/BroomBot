...
RTStepper::RTStepper(uint8_t stepPin, uint8_t dirPin, uint8_t enablePin)
{
	...
	m_Stepper->setMaxSpeed(200.0f);
	m_enablePin = enablePin;
	m_Stepper->setPinsInverted(false,true,true);
	m_Stepper->setEnablePin(enablePin);
...
RTStepper::RTStepper(uint8_t stepPin, uint8_t dirPin, uint8_t enablePin, float speed, float acceleration)
{
	m_Stepper = new AccelStepper(AccelStepper::DRIVER, stepPin, dirPin);
	m_Stepper->setEnablePin(enablePin);
...
void RTStepper::SetPinsInverted(bool dir, bool step , bool enable)
{
	m_Stepper->setPinsInverted(dir, step, enable );
}
